...
public class ScreeningServiceTests {
    ...
    @BeforeEach
    void setup() {
        movie = new Movie("Transformers", "sci-fi", 30);
        room = new Room("bedroom", 10, 10);
        screening = new Screening(movie, room, LocalDateTime.of(2024, 12, 20, 19, 0));
    }
    ...
    void testScreeningCreation() {
        ...
        when(screeningRepository.existsByMovieTitleAndRoomNameAndStartTime(movie.getTitle(), room.getName(),
                screening.getStartTime())).thenReturn(false);

        var result = screeningService.createScreening(movie.getTitle(), room.getName(),
    ...
    void testScreeningCreationFailsWhenScreeningAlreadyExists() {
        when(screeningRepository.existsByMovieTitleAndRoomNameAndStartTime(movie.getTitle(), room.getName(),
                screening.getStartTime())).thenReturn(true);
    ...
    void testScreeningCreationFailsOnOverlap() {
        var overlappingScreening1 = new Screening(movie, room, LocalDateTime.of(2024, 12, 20, 18, 35));
        var overlappingScreening2 = new Screening(movie, room, LocalDateTime.of(2024, 12, 20, 20, 0));
        when(screeningRepository.existsByMovieTitleAndRoomNameAndStartTime(movie.getTitle(), room.getName(),
                screening.getStartTime())).thenReturn(false);
        when(movieRepository.findByTitle(movie.getTitle())).thenReturn(Optional.of(movie));
        when(roomRepository.findByName(room.getName())).thenReturn(Optional.of(room));
        when(screeningRepository.findAllByRoomName(room.getName())).thenReturn(List.of(overlappingScreening1,
                overlappingScreening2));

        var result = screeningService.createScreening(movie.getTitle(), room.getName(),
                Screening.TIME_FORMAT.format(screening.getStartTime()));
    ...
    void testScreeningCreationFailsOnBreakOverlap() {
        ...
        when(movieRepository.findByTitle(movie.getTitle())).thenReturn(Optional.of(movie));
        when(roomRepository.findByName(room.getName())).thenReturn(Optional.of(room));
        when(screeningRepository.findAllByRoomName(room.getName())).thenReturn(List.of(new Screening(movie, room,
                LocalDateTime.of(2024, 12, 20, 18, 25))));

        var result = screeningService.createScreening(movie.getTitle(), room.getName(),
                format(screening.getStartTime()));
        assertThat(result.isOk()).isFalse();
        assertThat(result.error()).isInstanceOf(OperationException.class);
        assertThat(result.error().getMessage()).isEqualTo("This would start in the break period after another " +
    ...
    void testScreeningDeletion() {
        ...
        var result = screeningService.deleteScreening(movie.getTitle(), room.getName(),
                format(screening.getStartTime()));
        assertThat(result.isOk()).isTrue();
